...
ytdl.getInfo = function getInfo(url, args, options, callback) {
    ...
    var defaultArgs = ['--dump-json', '--ffmpeg-location', ffmpegBinary];
 ...
 * @param {String} path
 */
ytdl.setPath = function setPath(pathname) {
    var path = require('path');
    if (process.platform == "win32") {
        ytdlBinary = path.join(pathname, "win32/youtube-dl.exe");
        ffmpegBinary = path.join(pathname, "win32/ffmpeg.exe");
    } else if (process.platform == "linux") {
        ytdlBinary = path.join(pathname, "linux/youtube-dl");
        ffmpegBinary = path.join(pathname, "linux/ffmpeg");
    } else if (process.platform == "darwin") {
        ytdlBinary = path.join(pathname, "darwin/youtube-dl");
        ffmpegBinary = path.join(pathname, "darwin/ffmpeg");
    }
};